...
9. **Moonlight Stream** (`id: "moonlight-stream"`, value-typed by `MoonlightSessionID`, one per session) — `MoonlightStreamView`, 1920x1080 default (`#if MOONLIGHT_ENABLED`)
10. **Moonlight Keyboard** (`id: "moonlight-keyboard"`, value-typed by `MoonlightSessionID`) — `MoonlightKeyboardView`, 1180x540 (`#if MOONLIGHT_ENABLED`)
...
`VNCConnectionManager`, `AudioStreamManager`, `MacNativeStreamManager`, and `MoonlightSessionStore` are injected via `.environment()`. Connection type routing happens in `ConnectionListView` — VNC and regular Native connections open their windows as plain siblings; a Unity-enabled Native connection opens only Unity Controls and keeps the desktop stream unsubscribed until requested there. Moonlight picks a session from the store (the one already bound to that connection, else a free one), presents `MoonlightPairingView` as a sheet bound to that session's manager, and the sheet opens that session's stream window on launch. Each Moonlight scene injects its session's `MoonlightConnectionManager`, so the stream and keyboard views read a single manager from the environment exactly as they did when there was only one.
...
| Type | Role |
|------|------|
| `MoonlightSessionStore` | `@Observable`. Owns one `MoonlightConnectionManager` per linked copy of moonlight-common-c (`MoonlightLibrary.count`, currently 3) and picks the session for a connection. Mirrors `MoonlightInputFocus` (which session the app-wide gamepad/mouse/keyboard go to) for views. |
| `MoonlightLibrary` | One linked copy of moonlight-common-c: its slot, a table of its entry points (`Functions`, one file per copy: `MoonlightLibrarySlotN.swift`) and the renderer/delegate references its C callbacks read. Everything that used to call a bare `LiSend…` calls the manager's `library` instead. See KNOWN_CONSTRAINTS for why the copies exist and how they are built. |
| `MoonlightConnectionManager` | `@Observable`. Central orchestrator for one session — state machine (idle → connecting → pairing → ready → streaming), owns all sub-components, manages CADisplayLink for frame delivery. Bound to one `MoonlightLibrary` for life. |
...
| `MoonlightStreamBridge` | C callback marshalling layer. `MoonlightBridge.makeCallbacks(slot:)` builds one set of non-capturing `@convention(c)` closures per copy (the slot is a literal in each — a C function pointer carries no context), all funnelling into slot-parameterised functions that read that copy's `MoonlightLibrary` renderers. |
...
| `MacNativeVideoCapability` (shared) | Asks VideoToolbox whether *this* device hardware-decodes HEVC Main 4:2:2 10-bit, by requiring a hardware decompression session for a real Main42210 format description. The viewer reports the answer in its hello; hosts send 4:2:0 to anything that doesn't claim it. |
| `MacHEVCEncoder` (macOS) | Realtime VideoToolbox HEVC encoder, alpha and chroma optional (4:2:0, or 4:2:2 10-bit for a viewer that proved it can decode it). Requires a hardware encoder and verifies it got one. Per-window streams keep alpha (`muxa`); the whole-display desktop stream does not (`hvc1`) — it covers every pixel, so a constant alpha plane is pure cost. Sends the exact big-endian CoreMedia image description before compressed frames, so whichever format the encoder produced survives transport, alpha-layer metadata included. |
...
| `MacNativeStreamClient` (all clients) | Receives framed format/video/replacement messages and serializes renderer work onto the main queue. |
| `MacNativeVideoRenderer` (all clients) | Reconstructs the transported `hvc1` format with `ContainsAlphaChannel`, creates compressed `CMSampleBuffer`s, and submits them through `AVSampleBufferVideoRenderer`. |
| `MacNativeStreamManager` (all clients) / `NativeStreamView` (visionOS), `MacNativeStreamWindowView` (macOS), `MobileNativeStreamView` (iOS) | The manager owns connection state, the desktop display layer, the per-window sessions and the two keyboard channels; connection generations prevent stale callbacks from tearing down a replacement connection. The visionOS view adds the per-window controller UI (inventory picker); the Mac view forwards `NSEvent`s (kVK codes verbatim to a macOS host, via `MacKeyCodeMap`'s inverse to a Windows one); the iOS view adds zoom and pan (`MobileViewport`). |
| `MacNativeWindowStreamCoordinator` (macOS) | Publishes the streamable-window inventory and owns per-window `SCContentFilter(desktopIndependentWindow:)` streams + encoders for the active v2 viewer (6-stream cap, area-scaled bitrate, resize tracking). Polls at 1 s while a window is streaming — that poll is what notices a resize, close or Space change — and at 5 s when none is, so an audio-only session isn't enumerating windows every second; the cadence switches the moment the stream set crosses empty. The inventory is sorted (app, then window ID) so the window server's front-to-back order can't reshuffle the picker, and a change that only moves the `isFocused` highlight republishes at most every 5 s. |
