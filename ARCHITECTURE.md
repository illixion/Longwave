...
7a. **Unity Controls** (`id: "mac-native-unity-controls"`, value-typed by `MacNativeUnityControlID.shared`) — persistent session controls and window switcher; reconciles the host inventory into up to six automatic per-window scenes and can surface the full desktop, 980x180 default
...
`VNCConnectionManager`, `AudioStreamManager`, `MacNativeStreamManager`, and `MoonlightConnectionManager` are injected via `.environment()`. Connection type routing happens in `ConnectionListView` — VNC and Native connections open their windows as plain siblings; a Unity-enabled Native connection opens Unity Controls even when the full desktop and audio views are disabled. Moonlight presents `MoonlightPairingView` as a sheet which opens the stream window on launch.
