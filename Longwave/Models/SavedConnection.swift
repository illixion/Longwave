...
enum ConnectionType: String, CaseIterable, Codable {
    case vnc
    /// Streams from the Longwave Companion menu bar app: Screen (native
...
final class SavedConnection {
    ...
    /// Static auth token presented to the Longwave Companion, shared by both
    ...
    /// Unused since immersion became the PC's to decide — the headset follows the
    /// host's alpha-blend telemetry instead of a stored preference, so there is
