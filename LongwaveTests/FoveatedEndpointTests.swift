...
final class FoveatedEndpointTests: XCTestCase {
    ...
    func testRetiredRemoteModeFallsBackToAutomatic() {
        ...
        // The stored column survives without the feature compiled in; the
        // accessor that reinterprets it does not, so only assert on it there.
        #if FOVEATED_ENABLED
        XCTAssertEqual(connection.foveatedConnectionMode, .systemDiscovered)
        #else
        XCTAssertEqual(connection.foveatedConnectionModeStorage, "remote")
        #endif
    }
