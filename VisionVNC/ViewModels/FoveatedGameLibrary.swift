...
final class FoveatedGameLibrary {
    ...
    private let log = Logger(subsystem: "com.illixion.VisionVNC", category: "GameLibrary")
    ...
    private static let noHostMessage =
        "The PC has not announced itself. The VisionVNC session broker has to be running on the "
        + "host — start it from the Windows companion, then try again."
    private static let noAnswerMessage =
        "The PC did not answer. Check that the VisionVNC Windows Companion is running."
